...
import { PgBossInspector } from './components/pg-boss-inspector';
import { SyntheticPayloadDispatcher } from './components/synthetic-payload-dispatcher';
...
function IntegrationsDashboardPage() {
  ...
  const [activeTab, setActiveTab] = React.useState<ActiveTab>('pg_boss');
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
        ...
        <div>
          <div className="flex items-center space-x-2.5">
            ...
            <h1 className="text-2xl font-bold text-slate-900">
              Diagnostics, Background Jobs & Integrations
            </h1>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Monitor partner health, inspect pg-boss queues, simulate latency and dispatch synthetic test payloads.
          </p>
        </div>
      ...
      <div className="space-y-4 pt-4 border-t border-slate-200">
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-2">
          <button
            type="button"
            onClick={() => setActiveTab('pg_boss')}
            className={`px-3.5 py-2 text-xs font-bold rounded-xl flex items-center space-x-1.5 transition-all ${
              activeTab === 'pg_boss' ? 'bg-[#12C395] text-slate-950 shadow-xs' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
            <Cpu className="w-3.5 h-3.5" />
            <span>pg-boss Job Inspector</span>
          </button>

          <button
            type="button"
            onClick={() => setActiveTab('synthetic_dispatcher')}
            className={`px-3.5 py-2 text-xs font-bold rounded-xl flex items-center space-x-1.5 transition-all ${
              activeTab === 'synthetic_dispatcher' ? 'bg-[#12C395] text-slate-950 shadow-xs' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
            <Send className="w-3.5 h-3.5" />
            <span>Synthetic Test Dispatcher</span>
          </button>
        ...
        {/* Tab Contents */}
        {activeTab === 'pg_boss' && <PgBossInspector />}
        {activeTab === 'synthetic_dispatcher' && <SyntheticPayloadDispatcher />}
